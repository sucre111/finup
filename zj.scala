...
object ZJFeature {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val ploan_m5 = ploan.filter($"create_time".gt(lit("2017-06-01"))).filter($"create_time".lt(lit("2017-07-01"))).select("apply_no", "create_time", "pass_time", "status", "is_over_due", "source", "user_id", "core_customer_id", "core_request_id")
    ...
    val apply_opinfo = m5_fk_phone.toDF("p_apply_no", "p_create_time", "p_pass_time", "p_status", "p_is_over_due", "p_source", "p_user_id", "p_core_customer_id", "p_core_request_id", "p_id", "p_id_no", "p_mobile").join(vi_dD, $"p_id_no" === $"id_no", "left").filter("p_create_time > start_time")
...
//    apply_join_zj.write.parquet("hdfs://192.168.176.52:8020/user/cloud/neo4j/wxin/data/zj/apply_join_zj280_m5")
    ...
    applyOpInfoFeature.repartition(1).write.csv("hdfs://192.168.176.52:8020/user/cloud/neo4j/wxin/data/zj/applyOpInfoFeature_m6")
